# version 2.1 of space allocation
...
# and now we are going to have a function to get maximum possible KPI for a given amount of space
#
# logic is as follows
...
# function to initialize Incremental KPI dataframe

initialize_incremental_kpi <- function(SPACE_ELASTICITY_PARAMS_DB)
{
  INCREMENTAL_KPI_DF <- data.frame(PRODUCT_ID=SPACE_ELASTICITY_PARAMS_DB$PRODUCT_ID,
                                   ...
                                   INCREMENTAL_KPI=sapply(SPACE_ELASTICITY_PARAMS_DB$PRODUCT_ID,
                                                          get_incremental_kpi,
                                                          allocated_space = 0,
                                                          SPACE_ELASTICITY_PARAMS_DB = SPACE_ELASTICITY_PARAMS_DB),
                                   stringsAsFactors = F)
  INCREMENTAL_KPI_DF
}
...
# else FALSE
all_space_allocated <- function(ALLOCATED_SPACE)
{
  # some issue here
  # print(str(ALLOCATED_SPACE))
...
get_incremental_kpi <- function(product_id, allocated_space,SPACE_ELASTICITY_PARAMS_DB,increment=1)
{
...
optimise_kpi <- function(total_space)
{
  ...
  INCREMENTAL_KPI <- initialize_incremental_kpi(SPACE_ELASTICITY_PARAMS_DB)

  # lets start to allocate
  while(!all_space_allocated(ALLOCATED_SPACE))
  {
    ...
    next_incremental_kpi <- get_incremental_kpi(product_id = max_pid,
                                                allocated_space = current_space_allocation,
                                                SPACE_ELASTICITY_PARAMS_DB = SPACE_ELASTICITY_PARAMS_DB
    )
  ...
  return(list("MAX_POSSIBLE_KPI"=sum(ALLOCATED_SPACE$INCREMENTAL_KPI),
              "SPACE_COUNT"=total_space,
              "SPACE_ALLOCATION"=ALLOCATED_SPACE))
}
